...
const schema = yup.object({
  title: yup.string().required("Value is mendatory"),
  date: yup.string().required("Value is mendatory"),
  note: yup.string().required("Value is mendatory"),
});
...
const Page = () => {
  ...
  const [requestReport, result] = useMutation(REQUEST_REPORT);

  const res = useQuery(GET_REPORT_SUMMARY);
  ...
  useEffect(() => {
    ...
    if (resp.data?.getReportsAndFiles?.code) {
      ...
      console.log(resp.data?.getReportsAndFiles?.data);
      setFilesReports(resp.data?.getReportsAndFiles?.data);
      console.log(resp.data?.getReportsAndFiles?.pagination);
      setPages(resp.data?.getReportsAndFiles?.pagination?.pages);
    }
  }, [resp.data]);

  useEffect(() => {
    if (res.data?.getReportSummary?.code) {
      // reset();
      toast.error(res.data?.getReportSummary?.message);
    } else if (res.data?.getReportSummary?.data) {
      console.log(res.data?.getReportSummary?.data);
      setSummary(res.data?.getReportSummary?.data);
    }
  }, [res.data]);

  useEffect(() => {
    console.log(result.data?.requestReportFromAccountManager);

    if (result.data?.requestReportFromAccountManager?.code) {
      // reset();
      toast.error(result.data?.requestReportFromAccountManager?.message);
    } else if (result.data?.requestReportFromAccountManager?.data?.id) {
      console.log(result.data?.requestReportFromAccountManager?.data?.id);
      reset();
      ...
      <div className="flex justify-between">
        <div>
          ...
          <p className="text-[#414141] text-[18px] leading-[28px]">
            Effortlessly track and manage all your important files and reports
          </p>
        </div>
        ...
        >
          <div className="bg-white h-[388px] w-[517px] rounded-[16px]">
            <div className="flex flex-col pt-[65px] gap-y-4 items-center h-full">
              ...
              <div className="max-w-[278px] mb-4 text-center text-[#04050F] font-normal text-[16px] leading-[24px]">
                Your file request has been sent to your account manager{" "}
              </div>
              <div className="flex gap-x-2">
                <Button
                  type="button"
