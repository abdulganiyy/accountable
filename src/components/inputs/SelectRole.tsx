...
const SelectRole: FC<SelectRoleProps> = ({
  ...
  const [current, setCurrent] = useState<any>("Viewer");

  const options = [
    { value: "Viewer", key: "Viewer", helperText: "What access they have" },
    { value: "Editor", key: "Editor", helperText: "What access they have" },
    { value: "Admin", key: "Admin", helperText: "What access they have" },
  ];
  ...
  return (
    <div className="flex flex-col gap-y-2 font-normal text-sm">
      ...
      <div className="border-[1px] border-[#EAEDEF] rounded-md cursor-pointer h-[48px]">
        ...
        >
          {show && (
            <div className="absolute top-full z-50 flex flex-col w-[231px] h-[177px] gap-y-2 text-black right-0 border-2 p-2 bg-white border-[#EAEDEF] rounded-[16px]">
              {options?.map((option: any) => {
                return (
                  ...
                  >
                    {option.key}
                    <span className="text-[#4C5259]">{option.helperText}</span>
                  </span>
                );
              })}
            </div>
